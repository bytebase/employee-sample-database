...
CREATE OR REPLACE VIEW  v_full_employee
AS
SELECT
    emp_no,
...
CREATE OR REPLACE VIEW v_full_department
AS
SELECT
...
--
-- shows the department with the number of employee
-- per department
--
...
drop function if exists employee_usage //
drop procedure if exists employee_help //

CREATE FUNCTION employee_usage ()
RETURNS TEXT
DETERMINISTIC
BEGIN
    RETURN
'
    ...
    PROCEDURE show_department()

        shows the department with the manager and
...
create procedure employee_help()
deterministic
begin
